...
import 'package:spend_io_app/features/wallet/data/mock/account_transactions_mock.dart';
...
class TransactionLedgerState {
  final List<RecentTransactionModel> allTransactions;
  final List<RecentTransactionModel> filteredTransactions;
...
class AccountDetailsViewModel extends ChangeNotifier {
  ...
  void addTransaction(RecentTransactionModel tx) {
    if (_ledgerState == null) {
      return;
    }
    
    final updatedList = List<RecentTransactionModel>.from(_ledgerState!.allTransactions)..add(tx);
  ...
  void _updateLedger() {
    ...
    final List<RecentTransactionModel> filtered = _ledgerState!.allTransactions.where((tx) {
      // 1. Apply Date Filter
      ...
      if (_filterState.activeRangeLabel == 'Today') {
        ...
        matchesDate = tx.date.year == lastMonthYear && tx.date.month == lastMonthVal;
        ...
        matchesDate = txDateZero.isAfter(thirtyDaysAgo.subtract(const Duration(seconds: 1)));
      }
